...
public class WSClients implements ConfigsListener {
	...
	private void initBindingProvider(BindingProvider bindingProvider, String url) {
		int requestTimeout = (int) Configs.API_WS_REQUEST_TIMEOUT.getValue();
		int connectTimeout = (int) Configs.API_WS_CONNECT_TIMEOUT.getValue();
	...
	public void init() {
		loadURLs();
		configsManagerBean.registerConfigsListener(ModulesEnum.SMSAPIManagement, this);
		...
		try {
			reportManagementServicePorts = new ThreadLocal<ReportManagementService>() {
				...
				protected ReportManagementService initialValue() {
					String reportServiceURL = (String) Configs.API_REPORT_WEBSERVICE_URL.getValue();
