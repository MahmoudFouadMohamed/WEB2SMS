...
//    private final String pattern = "yyyy-MM-dd_HH-mm-ss";
    ...
    public void init() {
        ...
        try {
            ...
            AppSettings.BaseDir.setEnvEntryValue(Configs.PROV_SERVICE_BASE_DIR.getValue().toString());
    ...
    public void close() {
        ...
        try {
            for (LoggersEnum loggerEnum : LoggersEnum.values()) {
                if (loggerEnum != LoggersEnum.PROV_UTILS) {
            ...
            appLogger.info("Closing appender " + LoggersEnum.PROV_UTILS.name() + "_appender");
            configuration.getLoggerConfig(LoggersEnum.PROV_UTILS.name()).stop();
            configuration.getAppender(LoggersEnum.PROV_UTILS.name() + "_appender").stop();
            appLogger.info("Success close appender " + LoggersEnum.PROV_UTILS.name() + "_appender");
        } catch (Exception e) {
