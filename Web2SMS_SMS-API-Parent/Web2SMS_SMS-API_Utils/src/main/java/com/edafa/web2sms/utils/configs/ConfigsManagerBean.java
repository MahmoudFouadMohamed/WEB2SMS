...
public class ConfigsManagerBean implements ConfigsManagerBeanLocal {
	...
	public boolean isConfigurationLoaded(ModulesEnum module) {
		Boolean result1 = configuratoinLoadedFlags.get(ModulesEnum.SMSAPI);
		...
		for (Iterator<Config> iterator = configs.iterator(); iterator.hasNext();) {
			...
			try {
				...
				case INTEGER:
					...
					if(config.equals(Configs.WS_CLIENT_REQUEST_TIMEOUT))
					{
						// call client to reset client request time out
						xmlHttpClient.refreshReadTimeOut();
					}else if(config.equals(Configs.WS_CLIENT_CONNECT_TIMEOUT))
					{
						// call client to reset client connect time out
						xmlHttpClient.refreshConnectTimeOut();
