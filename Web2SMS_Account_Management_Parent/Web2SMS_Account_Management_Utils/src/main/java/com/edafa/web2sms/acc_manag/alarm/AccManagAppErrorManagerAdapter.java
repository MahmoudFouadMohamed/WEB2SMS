...
import com.edafa.web2sms.acc_manag.alarm.AlarmRasingBean;
...
/**
 * Session Bean implementation class AccManagAppErrorManagerAdapter
 */
@Singleton
@Startup
@LocalBean
@DependsOn({"AccManagLoggingManagerBean", "AppErrorDao", "AccManagConfigsManagerBean"})
public class AccManagAppErrorManagerAdapter implements AppErrorManagerLocal, ConfigsListener {
     ...
     * Default constructor.
     */
    public AccManagAppErrorManagerAdapter() {
        dbAlarmDefinitions = new ArrayList<ErrorDefinitionEntity>();
    }

    @PostConstruct
    public void init() {
        configsManagerBean.registerConfigsListener(AccountManagModulesEnum.AccountManagUtils, this);
        setAppErrorsIds();
        initAlarmHandler();
    }
    ...
    public boolean initAlarmHandler() {
        if (!configsManagerBean.isConfigurationLoaded(AccountManagModulesEnum.AccountManagUtils)) {
            appLogger.warn("Alarms configs is not loadded, cannot initalize AppErrorEntity Handler");
            return false;
        }

        Logger alarmLogger = LogManager.getLogger(LoggersEnum.ACCOUNT_MNGMT_UTILS.name());
        String alarmPID = (String) Configs.ACCOUNT_MANAGEMENT_ALARMING_IDENTIFIER.getValue();
    ...
    @Override
    public void configurationRefreshed(AccountManagModulesEnum module) {
        refreshAlarmDefinitions();
    }
