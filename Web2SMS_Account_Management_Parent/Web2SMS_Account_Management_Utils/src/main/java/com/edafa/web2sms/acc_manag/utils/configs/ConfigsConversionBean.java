...
public class ConfigsConversionBean {
	...
	public Config getConfigModel(Configuration conf) {
		...
		try {
			Configs enumConfig = Configs.getConfig(AccountManagModulesEnum.valueOf(conf.getModuleId().getName()), conf.getKey());
			if (enumConfig != null) {
	...
	public List<Configuration> getConfigs(List<Config> configs, AccountManagModulesEnum module) {
