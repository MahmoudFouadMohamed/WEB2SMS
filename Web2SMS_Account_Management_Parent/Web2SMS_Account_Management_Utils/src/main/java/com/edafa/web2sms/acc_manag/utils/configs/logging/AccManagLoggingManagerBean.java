package com.edafa.web2sms.acc_manag.utils.configs.logging;
...
import com.edafa.web2sms.acc_manag.utils.configs.logging.interfaces.LoggingManagerLocal;
...
/**
 * Session Bean implementation class AccManagLoggingManagerBean
 */
@Singleton
@Startup
@DependsOn("AccManagConfigsManagerBean")
public class AccManagLoggingManagerBean implements LoggingManagerLocal, ConfigsListener {
...
//			System.out.println("ReportingServerBaseDir: " + Configs.ReportingServerBaseDir.getValue());
    ...
    public void close() {
        ...
        try {
            for (LoggersEnum loggerEnum : LoggersEnum.values()) {
                if (loggerEnum != LoggersEnum.ACCOUNT_MNGMT_UTILS) {
            ...
            appLogger.info("Closing appender " + LoggersEnum.ACCOUNT_MNGMT_UTILS.name() + "_appender");
            configuration.getLoggerConfig(LoggersEnum.ACCOUNT_MNGMT_UTILS.name()).stop();
            configuration.getAppender(LoggersEnum.ACCOUNT_MNGMT_UTILS.name() + "_appender").stop();
            appLogger.info("Success close appender " + LoggersEnum.ACCOUNT_MNGMT_UTILS.name() + "_appender");
        } catch (Exception e) {
    ...
    @Override
    public void configurationRefreshed(AccountManagModulesEnum modulesEnum) {
