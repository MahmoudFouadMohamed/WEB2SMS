package com.edafa.web2sms.acc_manag.utils.sms;
...
import com.edafa.web2sms.acc_manag.utils.sms.exception.AccountManagInvalidSMSReceiver;
...
public class SMSUtils {
	...
	// Validations
	public static SenderType getSenderType(String sender) throws AccountManagInvalidSMSSender {
		if (sender == null || sender.isEmpty()  || sender.equals(""))
			throw new AccountManagInvalidSMSSender("", "Null or empty sender");
		...
		if (isAlphnum(sender)) {
			if (sender.length() <= alphaNumSenderLen) {
				...
				throw new AccountManagInvalidSMSSender(sender, "Invalid alphanumeric sender length");
			}
			...
			throw new AccountManagInvalidSMSSender(sender, "Invalid sender length");
		}
	}

	public static ReceiverType getReceiverType(String receiver) throws AccountManagInvalidSMSReceiver {
		if (validateNationalAddress(receiver)) {
			...
			throw new AccountManagInvalidSMSReceiver(receiver, "Invalid SMS receiver");
		}
	}
	...
	public static String formatAddress(String addr, AccountManagMsisdnFormat resultFormat) throws AccountManagInvalidMSISDNFormatException,
			AccountManagInvalidAddressFormattingException {

		AccountManagMsisdnFormat srcAddrFormat = getAddressFormat(addr);
		return formatAddress(addr, srcAddrFormat, resultFormat);
	}

	public static String formatAddress(String addr, AccountManagMsisdnFormat srcAddrFormat, AccountManagMsisdnFormat resultFormat)
			throws AccountManagInvalidAddressFormattingException {
		...
		case INTER_KEY:
			...
			case NATIONAL_KEY:
			default:
				throw new AccountManagInvalidAddressFormattingException(srcAddrFormat, resultFormat);
			}
			break;
		case INTER_PLUS:
			switch (resultFormat) {
	...
	protected static AccountManagMsisdnFormat getAddressFormat0(String address) {
		...
		AccountManagMsisdnFormat result = null;
		...
		if (address.startsWith(MSISDN.getMSISDN_INTER_KEY_CC())) {
			if (msisdnLen == MSISDN.getMSISDN_INTER_KEY_CC_LEN()
				...
				result = AccountManagMsisdnFormat.INTER_KEY_CC_LOCAL;
			}
			...
			if (msisdnLen == MSISDN.getMSISDN_INTER_PLUS_CC_LEN()
					&& address.matches("\\" + MSISDN.getMSISDN_INTER_PLUS_CC() + ndcRegex + "[0-9]{" + msisdnSNLen
							+ "}")) {
				result = AccountManagMsisdnFormat.INTER_PLUS_CC_LOCAL;
			}
			...
			if (msisdnLen == MSISDN.getMSISDN_CC_LEN()
				...
				result = AccountManagMsisdnFormat.INTER_CC_LOCAL;
			}
			...
			result = AccountManagMsisdnFormat.NATIONAL_KEY;
		} else if (msisdnLen == MSISDN.getMSISDN_NATIONAL_LEN()
				&& address.matches(ndcRegex + "[0-9]{" + msisdnSNLen + "}")) {
			result = AccountManagMsisdnFormat.NATIONAL;
		} else if (address.matches(MSISDN.getMSISDN_INTER_KEY() + "[1-9][0-9]{5,14}")) {
			result = AccountManagMsisdnFormat.INTER_KEY;
		} else if (address.matches("\\" + MSISDN.getMSISDN_INTER_PLUS() + "[1-9][0-9]{5,14}")) {
			result = AccountManagMsisdnFormat.INTER_PLUS;
		} 
//		else if (address.matches("[1-9][0-9]{5,14}")) {
//			result = AccountManagMsisdnFormat.INTER;
//		}

		return result;
	}

	public static AccountManagMsisdnFormat getAddressFormat(String address) throws AccountManagInvalidMSISDNFormatException {

		AccountManagMsisdnFormat result = getAddressFormat0(address);
		if (result == null)
			throw new AccountManagInvalidMSISDNFormatException();
		return result;
	}

	public static boolean validateNationalAddress(String address) {
		try {
			AccountManagMsisdnFormat addrFormat = getAddressFormat0(address);
			switch (addrFormat) {
			case NATIONAL_KEY:
			case NATIONAL:
				return true;
	...
	public static ReceiverType getReceiverType(AccountManagMsisdnFormat format) {
		switch (format) {
		case NATIONAL:
		case NATIONAL_KEY:
			return ReceiverType.NATIONAL;
	...
	public static void main(String[] arg) throws AccountManagInvalidSMSSender, AccountManagInvalidMSISDNFormatException, AccountManagInvalidSMSReceiver {
		...
		// switch (choose) {
		// case 1:
		// result = formatAddress(target, AccountManagMsisdnFormat.INTER_KEY_CC_LOCAL);
		// break;
		// case 2:
		// result = formatAddress(target, AccountManagMsisdnFormat.INTER_PLUS_CC_LOCAL);
		// break;
		// case 3:
		// result = formatAddress(target, AccountManagMsisdnFormat.INTER_CC_LOCAL);
		// break;
		// case 4:
		// result = formatAddress(target, AccountManagMsisdnFormat.NATIONAL_KEY);
		// break;
		// case 5:
		// result = formatAddress(target, AccountManagMsisdnFormat.NATIONAL);
		// break;
		// case 6:
		// result = formatAddress(target, AccountManagMsisdnFormat.INTER);
		...
		// }
		// } catch (AccountManagInvalidSMSReceiver e) {
