...
public class AppErrorManagerAdapter implements AppErrorManagerLocal, ConfigsListener {
    ...
    Logger cloudLogger = LogManager.getLogger(LoggersEnum.CLOUD_PROV.name());
    ...
    @PostConstruct
    public void init() {
        configsManagerBean.registerConfigsListener(ModulesEnum.ProvUtils, this);
    ...
    public boolean initAlarmHandler() {
        ...
        try {
            if (!configsManagerBean.isConfigurationLoaded(ModulesEnum.ProvUtils)) {
    ...
    public void raiseError(String logId, AppErrors error, String msg) {
        try {
            if (((Boolean) Configs.ERRORS_RAISING.getValue())) {
                AppError appError = new AppError(error.getId(), ErrorsSource.WEB_PROV.getSourceName(), msg);
                cloudLogger.debug(logId + " | Raising error: " + appError);
                AppErrMonitor appErrMonitor = alarmHandler.handleAppError(appError);

                if (appErrMonitor != null) {
                    cloudLogger.debug(logId + " | Error raised successfully");
                } else if (!checkAlarmsLoading()) {
                    synchronized (this) {
                        cloudLogger.error(logId + " | Alarms not loaded");
                        cloudLogger.info(logId + " | Loading alarms");
                        if (initAlarmHandler()) {
                            cloudLogger.info(logId + " | Alarms loaded successfully");
                            appErrMonitor = alarmHandler.handleAppError(appError);
                            if (appErrMonitor != null) {
                                cloudLogger.debug(logId + " | Error raised successfully");
                            } else {
                                cloudLogger.debug(logId + " | Failed to raise error: " + appError);
                            }
                        } else {
                            cloudLogger.error(logId + " | Error while loading Alarms");
                        }
                    }
                }
            }
        } catch (Exception e) {
            cloudLogger.error(logId + " | Unhandled Exception while raising error ");
            appLogger.error(logId + " | Unhandled Exception while raising error ", e);
        }
    }
