...
public class AppErrorManagerAdapter implements AppErrorManagerLocal, ConfigsListener {
	...
	public boolean initAlarmHandler() {
		if (!configsManagerBean.isConfigurationLoaded(ModulesEnum.Reporting)) {
	...
	private void registerErrorDefinitions(List<ErrorDefinitionEntity> dbAlarmDefinitionsList) {
		for (ErrorDefinitionEntity dbAlarmDefinition : dbAlarmDefinitionsList) {
			...
			try {
				...
				appLogger.info(
						"Failed to registered alarm definition: (" + alarmDefinition + "), Reason: " + e.getMessage());
			}
	...
	public boolean refreshAlarmDefinitions() {
		...
		appLogger.info(
				"All existing alarm definitions are unregistered from AppErrorEntity Handler, will register the new list");
	...
	public AppErrMonitor reportAppError(AppErrors error, String msg, ErrorsSource errorSource) {
