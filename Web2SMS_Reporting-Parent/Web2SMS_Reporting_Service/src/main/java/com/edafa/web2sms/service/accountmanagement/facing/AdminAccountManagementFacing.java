...
public class AdminAccountManagementFacing implements AdminAccountManagementFacingLocal {
	...
	public List<Account> findAllAccounts(AccountAdminTrxInfo trxInfo, int first, int count) throws DBException {
		...
		try {
			...
			try {
				...
				appLogger.error("Failed to return AdminAccountManagementBeanRemote to AccountManegementBeanRemotePool",
						ex);
			}
		}
	...
	@Override
	@TransactionAttribute(TransactionAttributeType.REQUIRED)
	public QuotaInfo getQuotaInfoByMSISDN(AccountTrxInfo trxInfo, String billingMSISDN)
			throws DBException, AccountNotFoundException, InvalidCustomerForQuotaInquiry, QuotaInquiryFailed,
			AccountManagInvalidMSISDNFormatException, AccountManagInvalidAddressFormattingException {
	...
	public List<Account> searchAccount(AccountAdminTrxInfo trxInfo, String accountID, String companyName,
		...
		try {
			accounts = adminAccountManagementBeanRemote.searchAccount(trxInfo, accountID, companyName, billingMSISDN,
					first, max);
		} finally {
			try {
				accountManegementRemotePools.returnAdminAccountManagementBeanRemote(adminAccountManagementBeanRemote);
	...
	@Override
	@TransactionAttribute(TransactionAttributeType.REQUIRED)
	public long countSearchAccount(AccountAdminTrxInfo trxInfo, String accountID, String companyName,
